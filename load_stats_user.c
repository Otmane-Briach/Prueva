...
//------------------------------DEFINES---------------------------------------//
#define MAP_SIZE 20
#define BPF_OBJ_FILE "rLEDBAT_kern.o"
...
void update_state_user_space(int fd_state, const char *new_state) {
    ...
    // Copia la cadena new_state a state_value,
    //pero nos aseguramos de no desbordar el buffer.
    snprintf(state_value, MAX_SIZE, "%s", new_state);

    if (bpf_map_update_elem(fd_state, &key, state_value, BPF_ANY)) {
        fprintf(stderr, "Error al actualizar el mapa state_map\n");
        return;
    }
}
...
//-------------------------OBTENCION DE LOS DESCRIPTORES DE ARCHIVO-----------------//
	int fd_values = bpf_obj_get(VALUES_MAP_PATH);
	if (fd_values < 0) {
		fprintf(stderr, "Error al abrir el mapa: %s\n", VALUES_MAP_PATH);
		return EXIT_FAILURE;
	}
  ...
  int fd_tsval_rtt_array = bpf_obj_get(TSVAL_RTT_ARRY_MAP_PATH);
  if (fd_ssthresh < 0) {
    fprintf(stderr, "Error al abrir el mapa: %s\n", TSVAL_RTT_ARRY_MAP_PATH);
    return EXIT_FAILURE;
  }
  int fd_time_rtt_array = bpf_obj_get(TIME_RTT_ARRAY_MAP_PATH);
  if (fd_ssthresh < 0) {
    fprintf(stderr, "Error al abrir el mapa: %s\n", TIME_RTT_ARRAY_MAP_PATH);
    return EXIT_FAILURE;
  }

//-----------------------------------INITIALICIAR----------------------------//
  //INITIALIZAR LAS_QD_VALUES
  value = MAX_LONG_LONG;
  for (key = 0; key < MAP_SIZE; key++) {
      if (bpf_map_update_elem(fd_values, &key, &value, BPF_ANY)) {
          fprintf(stderr, "Error al actualizar el mapa: %s\n", VALUES_MAP_PATH);
          return EXIT_FAILURE;
      }
  }
  //INITIALIZAR RTT_MIN
...
//------------------------------
value = 0;
for (key = 0; key < MAX_COUNT_RTT; key++) {
    if (bpf_map_update_elem(fd_tsval_rtt_array, &key, &value, BPF_ANY)) {
        fprintf(stderr, "Error al actualizar el mapa: %s\n", TSVAL_RTT_ARRY_MAP_PATH);
        return EXIT_FAILURE;
    }
}
for (key = 0; key < MAX_COUNT_RTT; key++) {
    if (bpf_map_update_elem(fd_time_rtt_array, &key, &value, BPF_ANY)) {
        fprintf(stderr, "Error al actualizar el mapa: %s\n", TIME_RTT_ARRAY_MAP_PATH);
        return EXIT_FAILURE;
    }
}
...
//-----------------------------
struct rcwnd_ok_str rcwnd_ok_value={0,0};
key = 0;
...
FILE *fp;
 ...
 if (bpf_map_update_elem(fd_tcp_rmem, &key, &values, BPF_ANY)) {
		 fprintf(stderr, "Erro al Actualizar fd_tcp_rmem\n");
		 return EXIT_FAILURE;
 }
