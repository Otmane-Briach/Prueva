...
static void update_state(char *new_state) {
    ...
    for (int i = 0; i < MAX_SIZE; i++) {
        ...
        if (!new_state[i]) {
            break; // Stop copying once we hit the null terminator
    }
  }
}
...
//------------------------------------------------
  ...
  if(!window_scales_ptr  || !current_ack_ptr
    ...
    || !flag_dup_ack_ptr ||!rcwnd_ok_ptr ||!key_timestamps_ptr)
  return TC_ACT_OK;
  ...
  *key_timestamps_ptr %= MAX_COUNT_RTT;
  ...
  __u64 tsval_value = timestamp_ptr->tsval;
...
/*
bpf_printk("write;%llu;%llu;%d;", rcwnd_ok_ptr->rcwnd_ok, *last_ack_ptr, seq_values->last_seq);
bpf_printk("%d;%llu;",  *tcp_rcwnd_ptr, *time_rtt_array_ptr);
bpf_printk("%d;%llu;",  *flag_dup_ack_ptr, window_scales_ptr->rcv_wscale);
*/
...
*/
    //----------------------------------
          ...
          if(*is_retransmission && !*recent_retransmission_ptr){
            ...
            //Se if there has been a packet loss
                ...
                *rcwnd_scale_ptr = (rcwnd_ok_ptr->rcwnd_ok - *ssthresh_ptr) * window_scales_ptr->window_scale;
   ...
   bpf_printk(" window :%lld, thresh :%lld,  State: %s, is_retransmission :%lld,  rcwnd_scale :%lld, reduction :%lld, queue_delay :%lld",rcwnd_ok_ptr->rcwnd_ok,*ssthresh_ptr, state, *is_retransmission, *rcwnd_scale_ptr, *reduction_ptr,*queue_delay_ptr);
